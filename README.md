...
```go
client, err := pop3.Dial(address,false)
...
func main() {
	if err := pop3.ReceiveMail("pop.163.com:995", "aa******@163.com", "******", true,
		func(number int, uid, data string, err error) (bool, error) {
			log.Printf("%d, %s\n", number, uid)
			r := strings.NewReader(data)
			// implement your own logic here
			e, err := enmime.ReadEnvelope(r)
			log.Println("Subject:", DecodeHeader(DecodeToUTF8Base64Header(e.Root.Header.Get("Subject"))))
